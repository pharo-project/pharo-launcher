    ...
    withEnv(["ARCHITECTURE=${architecture}"]) {
		node('linux') {
		    ...
		    stage("Packaging-user ${architecture}-bits") {
		    	sh './build.sh user'
		    	stash includes: 'build.sh, mac-installer-background.png, pharo-build-scripts/**, mac/**, windows/**, signing/*.p12, icons/**, launcher-version.txt, One/**', name: "pharo-launcher-one-${architecture}"
